// LinkGuard Firefox Extension - Popup Script
class LinkGuardFirefoxPopup {
  constructor() {
  ...
  async getSettings() {
    return new Promise((resolve) => {
      browser.runtime.sendMessage({ action: 'getSettings' }, resolve);
    });
  }
  ...
  bindEvents() {
    ...
    document.getElementById('openDashboard').addEventListener('click', () => {
      browser.tabs.create({ url: 'http://localhost:5173/dashboard' });
    });
    ...
    document.getElementById('helpBtn').addEventListener('click', () => {
      browser.tabs.create({ url: 'http://localhost:5173/help' });
    });
  ...
  async scanUrl(url) {
    return new Promise((resolve) => {
      browser.runtime.sendMessage({ action: 'scanUrl', url }, resolve);
    });
  }
  ...
  reportIssue() {
    const subject = encodeURIComponent('LinkGuard Firefox Extension Issue Report');
